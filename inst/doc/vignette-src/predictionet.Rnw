...
\noindent Out of the \Sexpr{length(genes.ras) * length(genes.ras)} possible interactions, only \Sexpr{sum(pn.priors.counts != 0, na.rm=TRUE)} have been reported at least once in our priors.
