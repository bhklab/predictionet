...
for (i in 1:nfold) {
	...
	mynet <- netinf(data=mydata[-s.ix, , drop=FALSE], perturbations=mypert[-s.ix, , drop=FALSE], priors=mypriors, priors.count=TRUE, priors.weight=0.5, maxparents=3, method="regrnet", seed=54321)
...
netinf.cv(data=mydata, categories=3, perturbations=mypert, priors=mypriors, maxparents=3, method="regrnet", nfold=10, seed=54321)
@

<<regrnetinfcvfun2,echo=FALSE,results=hide,eval=TRUE>>=
res <- netinf.cv(data=mydata, categories=3, perturbations=mypert, priors=mypriors, maxparents=3, method="regrnet", nfold=10, seed=54321)
...
boxplot(mypredscore.r2, ylim=c(0, 1), ylab="$R^2$ in cross-validation")
@
